...
local function RayCast(key)
	...
	if key.code == input.KEY.LeftAlt and I.UI.getMode() == "Container" and types.Player.quests(self)["MG_Sharn_Necro"].stage == 10 or types.NPC.record(self).class == "necromancer" and input.KEY.LeftAlt and I.UI.getMode() == "Container" then
		local faced = nearby.castRenderingRay(camera.getPosition(), camera.getPosition() + camera.viewportToWorldVector(util.vector2(0.5, 0.5))* 8192, {ignorePlayer = true})
			if faced.hitObject then 
				if types.NPC.objectIsInstance(faced.hitObject) then
					...
					core.sendGlobalEvent("SpawnProp", "ksn_ReanimateMenuObj")
					core.sendGlobalEvent("SetFacedActor", faced.hitObject)
					end
				end
			end
	end
